...
int main() {
  ...
  while (option != "Quit") {
    ...
    if (option == "Menu") {
      ...
      cout << " - PrintList." << setw(25) << "Print all players and scores."
           << endl;
      cout << " - FindPlayer." << setw(25) << "Search for a player's score."
           << endl;
      cout << " - AddPlayer." << setw(25) << "Add new player and score."
           << endl;
      ...
      outFile << "Player " << name << " has been added with a score of "
              << score << '.' << endl;
...
void printPlayers(ofstream &dataFile, int numPlayers, Player *players) {
  ...
  for (int i = 0; i < numPlayers; i++)
    dataFile << setw(2) << right << (i + 1) << ". " << left << setw(12)
             << players[i].getName() << setw(5) << right
